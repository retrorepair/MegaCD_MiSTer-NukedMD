...
module tb_mcd_int2;
   ...
   // ---- REVIEW instrumentation: INT2 ISR entries and INT_ACK(2) pulse shape ----
   // one entry = a fresh fetch of the level-2 ISR's first instruction at 0x000334
   int  int2_entries=0; logic in_isr2=0;
   always @(posedge MCLK) begin
      if (DBG_S68K_AS_N==1'b0 && DBG_S68K_A==24'h000334 && !in_isr2) begin int2_entries++; in_isr2<=1; end
      if (DBG_S68K_AS_N==1'b0 && DBG_S68K_A!==24'h000334) in_isr2<=0;
   end
   int  ack2_pulses=0, ack2_min=999999, ack2_max=0, ack2_run=0;
   logic ack2_old=0;
   always @(posedge CLK) begin
      if (dut.ASIC.INT_ACK[2]===1'b1) begin
         if (!ack2_old) begin ack2_pulses++; ack2_run=1; end else ack2_run++;
      end
      else if (ack2_old) begin
         if (ack2_run < ack2_min) ack2_min = ack2_run;
         if (ack2_run > ack2_max) ack2_max = ack2_run;
      end
      ack2_old <= (dut.ASIC.INT_ACK[2]===1'b1);
   end
   ...
   task automatic test_irq_int2(output int err);
      ...
      for (i = 0; i < 8; i++) begin
         ...
         ext_rd('h12026,1'b1,v);
         if (v[7:0]==8'd2) $display("    [irq] iter %0d: COMSTA3 still 2 ent_before the write", i);
   ...
   task automatic test_dma3(input int PTv, input bit fast, output int err);
      ...
      // exercised by either bench ent_before, and one the ccb6fdf DMA_BYTE change touches.
      cdc_end();
      ...
      // real test does not have, which let the transfer run ~134 bytes ent_before the DD rewrite
   ...
   initial begin
      ...
      // ==================== REVIEW: INT2 acknowledge-edge sweep (8c933ee) ====================
      // Two IFL2 requests separated by a swept delay that walks across the sub-CPU's IACK
      // window.  Legal outcomes per pair: 1 entry (the two coalesced ent_before the acknowledge)
      // or 2 (the second arrived after it).  0 = a lost interrupt; >2 = a double-take.
      // Afterwards INT_PEND(2)/IFL2 must both be 0 -- a stuck pending flag is an ISR storm.
      begin
         int d, ent_before, got, n0, n1, n2, nx;
         n0=0; n1=0; n2=0; nx=0;
         mcd_wr16('hFF8032, 16'h0004);   // IEN(2)=1 again: test_irq_int2 restored IEN(5) and cleared it
         for (d = 0; d < 60; d++) begin
            ent_before = int2_entries;
            ext_wr('h12000, 16'h0100, 1'b1, 1'b0);      // request #1
            repeat (d*8) @(posedge CLK);                // 0 .. ~8.8 us in 149 ns steps
            ext_wr('h12000, 16'h0100, 1'b1, 1'b0);      // request #2
            repeat (3000) @(posedge CLK);               // let both be serviced
            got = int2_entries - ent_before;
            case (got)
              0: begin n0++; $display("    [race d=%0d] LOST: 0 ISR entries for 2 requests", d); end
              1: n1++;
              2: n2++;
              default: begin nx++; $display("    [race d=%0d] EXTRA: %0d ISR entries for 2 requests", d, got); end
            endcase
            if (dut.ASIC.INT_PEND[2] !== 1'b0 || dut.ASIC.IFL2 !== 1'b0)
               $display("    [race d=%0d] STUCK: INT_PEND2=%b IFL2=%b after both serviced",
                        d, dut.ASIC.INT_PEND[2], dut.ASIC.IFL2);
         end
         $display("  [race] 60 pairs: lost=%0d coalesced(1)=%0d separate(2)=%0d extra(>2)=%0d", n0, n1, n2, nx);
         $display("  [ack ] INT_ACK(2) pulses=%0d  width min=%0d max=%0d CLK  (a 1-2 clock pulse would be a decode glitch)",
                  ack2_pulses, ack2_min, ack2_max);
      end
      $display("======== INT2 review done ========");
      $finish;
