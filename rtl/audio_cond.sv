...
//
// MegaCD: adds the external (Mega CD PCM/CDDA) input that is mixed in before the
// Genesis low-pass filter when "CD Audio: Filtered" is selected, the FM/PSG debug
// mutes, and the LPF sample enable used to time the audio output.
//
...
always @(posedge clk) begin
	reg [15:0] sms_fm;
	reg [15:0] al,ar;
	reg [15:0] fml,fmr,psgm;

	sms_fm <= ^sms_fm_audio[13:12] ? {{2{sms_fm_audio[13]}}, {12{sms_fm_audio[12]}}, 2'b00} : {sms_fm_audio[12],sms_fm_audio[12:0], 2'b00};

	fml  <= fm_en  ? ((lpf_mode == 1) ? md_fm_lpf_l : md_fm_l) : 16'd0;
	fmr  <= fm_en  ? ((lpf_mode == 1) ? md_fm_lpf_r : md_fm_r) : 16'd0;
	psgm <= psg_en ? ((lpf_mode == 3) ? psg_amp : psg) : 16'd0;
	...
	// Mega CD audio enters the Genesis mixer here when it should pass through the console's filter
	mix_l <= ext_en ? ({pre_lpf_l[15],pre_lpf_l[15:1]} + {ext_l[15],ext_l[15:1]}) : pre_lpf_l;
	mix_r <= ext_en ? ({pre_lpf_r[15],pre_lpf_r[15:1]} + {ext_r[15],ext_r[15:1]}) : pre_lpf_r;
end
