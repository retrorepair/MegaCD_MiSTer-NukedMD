...
entity MCD is
	port(
		...
		PCMRAM_A			: out std_logic_vector(15 downto 0);
		PCMRAM_DO		: out std_logic_vector(7 downto 0);
		PCMRAM_DI		: in std_logic_vector(7 downto 0);
		PCMRAM_RD		: out std_logic;
		PCMRAM_WR		: out std_logic;
		PCMRAM_BUSY		: in std_logic;
...
architecture rtl of MCD is
	...
	signal PCM_RAM_RD_A	: std_logic;
	signal PCM_RDY			: std_logic;
...
begin
	...
	port map(
		clk			=> CLK,
		reset			=> not RST_N,
		cpu_addr		=> PCM_RAM_ADDR_A,
		cpu_din		=> PCM_RAM_DO_A,
		cpu_wr		=> PCM_RAM_WE_A,
		cpu_rd		=> PCM_RAM_RD_A,
		cpu_dout		=> PCM_RAM_DI_A,
		cpu_rdy		=> PCM_RDY,
		...
		mem_rd		=> PCMRAM_RD,
		mem_wr		=> PCMRAM_WR,
		mem_dout		=> x"00" & PCMRAM_DI,
		mem_busy		=> PCMRAM_BUSY
	);
