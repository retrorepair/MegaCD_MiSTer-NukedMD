...
module md_board
	...
	// MegaCD: expansion connector (raw pin levels, active low as on the real edge connector)
	input ext_cart,           // /CART pin: 0 = cartridge inserted (CE0 at 000000), 1 = empty slot (expansion /ROM at 000000)
	input ext_disk,           // DISK pin of the I/O chip (version register bit 5): 0 = expansion unit attached
	output exp_as,            // /AS
	output exp_uds,           // /UDS
	output exp_lds,           // /LDS
	output exp_rw,            // R/W
	output exp_asel,          // /ASEL
	output exp_rom,           // /ROM  (expansion ROM area select)
	output exp_ras2,          // /RAS2 (expansion RAM area select)
	output exp_fdc,           // /FDC  (expansion register area select, A12000-A120FF)
...
`endif
	...
	reg MCLK_e = 1'h0; // MegaCD: explicit power-up value (simulation would otherwise stay X)
...
`endif
	...
	wire [15:0] exp_VD_d = {16{~exp_data_en}}; // MegaCD: expansion device data bus drive
	...
	begin
		VD <= 
			...
			(~cart_VD_d & cart_data) |
			(~exp_VD_d & exp_data) |
			((ym_VD_d & m68k_VD_d & ram_VD_d & cart_VD_d & exp_VD_d) & VD);
	...
	assign NTSC = ~pal;
	assign JAP = ~jap;
	assign DISK = ext_disk; // MegaCD: was 1'h1 (no expansion unit)
...
`endif
	...
	assign CART = ext_cart; // MegaCD: was 1'h0 (cartridge always present)

	// MegaCD: expansion connector
	assign exp_as = AS;
	assign exp_uds = UDS;
	assign exp_lds = LDS;
	assign exp_rw = RW;
	assign exp_asel = ASEL;
	assign exp_rom = ROM;
	assign exp_ras2 = RAS2;
	assign exp_fdc = FDC;
